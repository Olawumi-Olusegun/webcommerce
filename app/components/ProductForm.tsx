...
export default function ProductForm(props: Props) {
  ...
  const removeImage = async (index: number) => {
    ...
    if(imageToRemove.startsWith(cloudSourceUrl)) {
      ...
      // if this image is from the local state, we want to update the local state
      const fileIndexDifference = productImagesSource.length - imageFiles.length;
      const indexToRemove = index - fileIndexDifference;
      const newImageFiles = imageFiles.filter((_, idx) => {
        if(idx !== indexToRemove) return true;
      });
    ...
    const newImageSource = productImagesSource.filter((_, idx) => {
